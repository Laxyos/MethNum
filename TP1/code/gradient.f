...
c     VERSION AVEC PRODUIT MATRICE VECTEUR OPTIMISÉ
c     Méthode du gradient conjugué pour Ax = b
c     Renvoit le vecteur x0
      SUBROUTINE grad_conj2(A, b, n,x0)
      IMPLICIT NONE
...
c       Calcul de alpha et de la norme du résidu
       ...
       res = sqrt(rkScal)
       call produit2(A, d0,INT(sqrt(dble(n))), n, vecTemp)
...
c       Calcul de r_k+1
c       call produit(A, d0, n, Ad0)
       DO j = 1,n
       r0(j) = r0(j)-alpha*vecTemp(j)
       ENDDO
